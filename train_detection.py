...
def main():
    ...
    # 加载预训练的模型
    model = YOLO("ultralytics/cfg/models/11/yolo11.yaml").load("weights/yolo11x.pt")

    # 定义训练参数，添加默认值、范围和中文注释
    train_params = {
        'data': "ultralytics/cfg/datasets/coco.yaml",  # 数据集配置文件路径，需要自定义修改
        'epochs': 2000,  # 总训练轮次，默认值 100，范围 >= 1
        ...
        'close_mosaic': 50,  # 在最后 N 轮次中禁用 Mosaic 数据增强，默认值 10
